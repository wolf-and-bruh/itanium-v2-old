...
void utils::CreateDirectoryRec(std::string input)
{
...
std::string utils::GetDirectory(std::string input)
{
    size_t pos = 0;
    std::string Directory = "";
    while ((pos = input.find("/")) != std::string::npos) {
...
std::string utils::GetExtension(std::string input)
{
...
std::string utils::StrToAscii(std::string& input)
{
    std::string output = "";
    for (int i = 0; i < input.length(); i++)
        if (input[i] > ' ' && input[i] < '~')
...
std::string utils::Sanitize(std::string& input)
{
    size_t pos = std::string::npos;
...
std::string utils::readContent(std::string path)
{
...
std::string utils::CVarStr(uintptr_t address)
{
